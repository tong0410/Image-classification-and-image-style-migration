...
class VGGNet(nn.Module):
    def __init__(self):
        ...
        self.vgg = models.vgg16(pretrained = False).features  # 提取卷积层
...
# 加载图片
content_img = load_img("./golden_gate.jpg")
...
total_step = 10000 # 训练次数
style_weight = 20000 # style_loss的权重
...
for step in range(total_step):
    ...
    print("Step [{}/{}], content loss = {:.4f}, style loss = {:.4f}"
          .format(step+1, total_step, content_loss.item(), stype_loss.item()))

    while step == total_step - 1:
        toPIL = transforms.ToPILImage()
        img = toPIL(img)
        img.save('testout_424.jpg')
        break
